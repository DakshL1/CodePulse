...
const CodeExecutionArea = ({ roomId, testCases, code, language, setTestCases }) => {
  ...
  return (
    <div className="bg-white p-4 shadow-md rounded-lg flex-1 flex flex-col">
      <h2 className="font-semibold mb-2">Code Execution Area</h2>
          ...
          {testCases.length > 0 ? (
            <div className="space-y-2">
              {testCases.map((testCase, index) => (
                <div
                  key={index}
                  className={`p-2 rounded cursor-pointer ${selectedTestCase === index ? 'bg-blue-100 border border-blue-300' : 'bg-gray-50 hover:bg-gray-100'}`}
                  onClick={() => setSelectedTestCase(index)}
                >
                  <p className="text-xs font-medium">Test Case {index + 1}</p>
                  <p className={`text-xs ${testCase.status === "Success" ? "text-green-600" : testCase.status === "Failed" ? "text-red-600" : "text-gray-500"}`}>
        ...
        <div className="w-2/3 border rounded-lg p-2 flex flex-col">
          ...
          <div className="mt-2">
            <button
              ...
              className={`text-black px-3 py-1 rounded text-sm ${isExecuting || !testCases.length ? 'bg-gray-400' : 'bg-green-600 hover:bg-green-700'}`}
            >
            ...
            <div className="space-y-2 flex-1">
              ...
              {testCases[selectedTestCase]?.output && (
                <div className="bg-gray-50 p-2 rounded mt-2 flex-1 overflow-y-auto">
                  ...
                  <p className={`text-xs font-medium mt-1 ${testCases[selectedTestCase]?.status === "Success" ? "text-green-600" : "text-red-600"}`}>
